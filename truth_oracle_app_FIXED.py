...
if page == "🎯 Live Demo":
    ...
    if verify_button and event_query:
        with st.spinner("Verifying with multi-source consensus..."):
            ...
            # CRITICAL: Check if query is about the FUTURE (we can't predict, only verify current/past)
            future_keywords = [
                'will', 'will be', 'going to', 'gonna',
                'in the next', 'in 5 minutes', 'in 1 minute', 'in 10 minutes',
                'in an hour', 'in 1 hour', 'in a day', 'in 1 day',
                'tomorrow', 'next week', 'next month', 'next year',
                'by tomorrow', 'by next week',
                'future', 'predict', 'prediction'
            ]
            is_future_query = any(keyword in query_lower for keyword in future_keywords)

            if is_future_query:
                st.error("❌ **Cannot Verify Future Events**")
                st.warning("Truth Oracle verifies **current and historical data only**, not future predictions.")
                st.markdown("---")
                st.info("**💡 Try asking instead:**")
                st.markdown("✅ 'Is Bitcoin currently above $93,000?'")
                st.markdown("✅ 'Bitcoin above $93k' (checks current price)")
                st.markdown("✅ 'Ethereum price above $3,500'")
                st.markdown("---")
                st.markdown("**Why we don't predict the future:**")
                st.markdown("• Multi-source consensus verifies **facts** (current/past data)")
                st.markdown("• Predictions require ML models, not data verification")
                st.markdown("• Our edge is **accuracy on verifiable events**, not fortune-telling")
